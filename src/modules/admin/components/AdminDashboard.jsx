...
function AdminDashboard() {
  ...
  const [inventarioFormData, setInventarioFormData] = useState({
    nombre: '', descripcion: '', categoria: 1,
    cantidadActual: 0, cantidadMinima: 0, cantidadMaxima: 0,
    unidad: 'unidades', precioCosto: 0, fechaVencimiento: '', proveedor: ''
  })
  ...
  const [config, setConfig] = useState({
    capacidadMaxima: '', horaApertura: '', horaCierre: '',
    tiempoPromedioMesa: '', margenGanancia: ''
  })
  ...
  const [formData, setFormData] = useState({
    periodoInicio: '', periodoFin: '', consideraFestivos: true, consideraTendencias: true
  })
  ...
  const loadPrestadores = async () => {
    ...
    } catch { setPrestadoresError('No se pudo cargar la información de prestadores') }
    finally { setPrestadoresLoading(false) }
  }

  const loadReporteVentas = async (fechaInicio = filtroVentas.fechaInicio, fechaFin = filtroVentas.fechaFin) => {
    try {
      setReporteVentasLoading(true)
      const data = await getReporteVentas(fechaInicio, fechaFin)
      setReporteVentas(data.data || data)
    } catch { setReporteVentasError('No se pudo cargar el reporte de ventas') }
    finally { setReporteVentasLoading(false) }
  }
  ...
  const loadAuditoria = async () => {
    ...
    } catch { setAuditoriaError('No se pudo cargar la auditoría de actividades') }
  ...
  const loadInventario = async () => {
    ...
    } catch { setInventarioError('No se pudo cargar el resumen de inventario') }
    ...
    try {
      ...
      const arrayData = Array.isArray(listData) ? listData : (Array.isArray(listData?.data) ? listData.data : [])
      setInventarioLista(arrayData)
    } catch { setInventarioListaError('No se pudo cargar la lista de inventario') }
    finally { setInventarioListaLoading(false) }
  }
  ...
  const loadPredicciones = async () => {
    ...
    } catch { setPredError('No se pudieron cargar las predicciones') }
  ...
  const loadAdmins = async () => {
    try {
      ...
      setAdmins(data)
    } catch { setAdminsError('No se pudieron cargar los administradores') }
  ...
  const loadActiveAdmins = async () => {
    ...
    } catch { setActiveAdminsError('No se pudieron cargar los administradores activos') }
    finally { setActiveAdminsLoading(false) }
  }
  ...
  const handleSearchAdmins = async (e) => {
    ...
    try {
      ...
      setActiveAdmins(data)
    } catch { setActiveAdminsError('Error al buscar administradores') }
    finally { setActiveAdminsLoading(false) }
  }
  ...
  const handleConfigChange = (e) => {
    ...
    setConfigSuccess(''); setConfigError('')
  }

  const handleConfigSubmit = async (e) => {
    e.preventDefault(); setConfigError(''); setConfigSuccess('')
    try {
      setConfigLoading(true)
      await saveConfig({
    ...
    } catch { setConfigError('Error al guardar la configuración') }
    finally { setConfigLoading(false) }
  }
  ...
  const handleCreatePrediccion = async (e) => {
    ...
    if (!formData.periodoInicio || !formData.periodoFin) { setFormError('Debes indicar fecha de inicio y fin'); return }
    if (formData.periodoFin < formData.periodoInicio) { setFormError('La fecha de fin no puede ser anterior al inicio'); return }
    try {
      setFormLoading(true)
      await createPrediccion(formData)
      await loadPredicciones()
      setShowForm(false)
      setFormData({ periodoInicio: '', periodoFin: '', consideraFestivos: true, consideraTendencias: true })
    } catch (err) { setFormError(err.message || 'Error al generar predicción') }
    finally { setFormLoading(false) }
  }

  const handleAdminFormChange = (e) => {
    const { name, value, type, checked } = e.target
    setAdminFormData({ ...adminFormData, [name]: type === 'checkbox' ? checked : value })
    setAdminFormError('')
  }
  ...
  const handleCreateAdmin = async (e) => {
    ...
    if (!profile || !profile.id) { 
      setAdminFormError('No se pudo identificar tu usuario actual. Recarga la página.'); 
      return 
    }
    ...
    try {
      ...
      setAdminFormData({ usuarioId: '', nivelAcceso: 'SuperAdmin', areaResponsabilidad: 'Operaciones generales', activo: true })
    } catch (err) { setAdminFormError(err.message || 'Error al crear administrador') }
    finally { setAdminFormLoading(false) }
  }

  const handleDeleteAdmin = async (id) => {
    if (!window.confirm('¿Está seguro de que desea eliminar a este administrador? Esta acción no se puede deshacer.')) return
  ...
  const handleCreateInventario = async (e) => {
    ...
    try {
      ...
      setInventarioFormError(err.message || 'Ocurrió un error al crear el producto')
    } finally {
    ...
    <div className={`adm-layout ${sidebarCollapsed ? 'collapsed' : ''}`}>
      ...
      <aside className="adm-sidebar">
        ...
        <div className="adm-brand">
          ...
          <button className="adm-collapse-btn" onClick={() => setSidebarCollapsed(p => !p)} title="Colapsar menú">
            {sidebarCollapsed ? '›' : '‹'}
          </button>
        </div>
        ...
        <nav className="adm-nav">
          ...
          {MENU_ITEMS.map(item => (
            ...
            >
              ...
              {!sidebarCollapsed && <span className="adm-nav-label-text">{item.label}</span>}
      ...
      <div className="adm-main">
            ...
            {profile ? (
              <>
                <div className="adm-user-info">
                  <p className="adm-user-name">{profile.firstName} {profile.lastName}</p>
                  <p className="adm-user-email">{profile.email}</p>
                </div>
                <div className="adm-avatar" title={`${profile.firstName} ${profile.lastName}`}>
                  {getInitials(profile)}
                </div>
              </>
            ) : (
        ...
        <main className="adm-content">
          ...
          {activeSection === 'estadisticas' && (
            <div className="panel-card" style={{ background: 'transparent', boxShadow: 'none', padding: 0 }}>
              <div className="panel-card-header" style={{ background: '#fff', borderRadius: '12px', padding: '24px', marginBottom: '24px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
                <div>
                ...
                <>
                  {stats && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', marginBottom: '24px' }}>
                      <div style={{ background: '#fff', padding: '24px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', border: '1px solid #f3f4f6' }}>
                        <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Total Reservas</h3>
                        <p style={{ fontSize: '32px', fontWeight: '700', color: '#111827' }}>{stats.totalReservas}</p>
                      </div>
                      <div style={{ background: '#fff', padding: '24px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', border: '1px solid #f3f4f6' }}>
                        <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Reservas Hoy</h3>
                        <p style={{ fontSize: '32px', fontWeight: '700', color: '#059669' }}>{stats.reservasHoy}</p>
                      </div>
                      <div style={{ background: '#fff', padding: '24px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', border: '1px solid #f3f4f6' }}>
                        <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Total Empleados</h3>
                        <p style={{ fontSize: '32px', fontWeight: '700', color: '#3b82f6' }}>{stats.totalEmpleados}</p>
                      </div>
                      <div style={{ background: '#fff', padding: '24px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', border: '1px solid #f3f4f6' }}>
                        <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Prods. Bajo Stock</h3>
                        <p style={{ fontSize: '32px', fontWeight: '700', color: stats.productosConBajoStock > 0 ? '#dc2626' : '#6b7280' }}>
                  ...
                  {reservasHoy && (
                    <>
                      <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#111827', margin: '32px 0 16px' }}>Detalles de Reservas (Hoy)</h3>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
                        <div style={{ background: '#fff', padding: '24px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', border: '1px solid #f3f4f6' }}>
                          <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Total de Personas Hoy</h3>
                          <p style={{ fontSize: '32px', fontWeight: '700', color: '#8b5cf6' }}>{reservasHoy.personasTotal}</p>
                        </div>
                        <div style={{ background: '#fff', padding: '24px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', border: '1px solid #f3f4f6' }}>
                          <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Ingreso Estimado Hoy</h3>
                          <p style={{ fontSize: '32px', fontWeight: '700', color: '#d97706' }}>${(reservasHoy.ingresoEstimado || 0).toLocaleString()}</p>
                        </div>
                      </div>
                    </>
                  )}
            ...
            <div className="panel-card" style={{ background: 'transparent', boxShadow: 'none', padding: 0 }}>
              ...
              {inventarioLoading ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px' }}>
                  {Object.entries(inventario).map(([key, value]) => {
                    ...
                    const isCurrency = key.toLowerCase().includes('valor') || key.toLowerCase().includes('costo') || key.toLowerCase().includes('ingreso') || key.toLowerCase().includes('precio');
              ...
              <div className="panel-card" style={{ padding: '0', overflow: 'hidden', marginTop: '24px' }}>
                ...
                {inventarioListaLoading ? (
                  <div className="loading-state">Cargando lista de inventario...</div>
                ) : inventarioLista && inventarioLista.length > 0 ? (
                  <div className="table-container">
                    <table className="data-table">
                      <thead>
                        <tr>
                          <th style={{ textAlign: 'left', padding: '16px 24px' }}>NOMBRE</th>
                          <th style={{ textAlign: 'left', padding: '16px 24px' }}>CATEGORÍA</th>
                          <th style={{ textAlign: 'right', padding: '16px 24px' }}>CANT. ACTUAL</th>
                          <th style={{ textAlign: 'right', padding: '16px 24px' }}>CANT. MÍNIMA</th>
                          <th style={{ textAlign: 'right', padding: '16px 24px' }}>PRECIO COSTO</th>
                          <th style={{ textAlign: 'left', padding: '16px 24px' }}>PROVEEDOR</th>
                          <th style={{ textAlign: 'left', padding: '16px 24px' }}>VENCIMIENTO</th>
                        </tr>
                      </thead>
                      <tbody>
                        {inventarioLista.map((item, idx) => (
                          <tr key={idx}>
                            <td style={{ padding: '16px 24px', fontWeight: '500' }}>{item.nombre}</td>
                            <td style={{ padding: '16px 24px', color: '#6b7280' }}>{item.categoriaNombre || `ID: ${item.categoria}`}</td>
                            <td style={{ textAlign: 'right', padding: '16px 24px' }}>{item.cantidadActual} {item.unidad}</td>
                            <td style={{ textAlign: 'right', padding: '16px 24px', color: item.cantidadActual <= item.cantidadMinima ? '#dc2626' : '#6b7280' }}>
                              {item.cantidadMinima} {item.unidad}
                            </td>
                            <td style={{ textAlign: 'right', padding: '16px 24px' }}>${(item.precioCosto || 0).toLocaleString()}</td>
                            <td style={{ padding: '16px 24px' }}>{item.proveedor || 'N/A'}</td>
                            <td style={{ padding: '16px 24px' }}>{item.fechaVencimiento ? new Date(item.fechaVencimiento).toLocaleDateString() : 'N/A'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          {activeSection === 'auditoria' && (
            <div className="panel-card" style={{ background: 'transparent', boxShadow: 'none', padding: 0 }}>
              ...
              {auditoriaError && <div className="error-message">{auditoriaError}</div>}
              ...
              <div className="panel-card" style={{ padding: '0', overflow: 'hidden' }}>
                <div className="table-container">
                  <table className="data-table">
                    <thead>
                      <tr>
                        <th style={{ textAlign: 'left', padding: '16px 24px' }}>FECHA</th>
                        <th style={{ textAlign: 'left', padding: '16px 24px' }}>USUARIO</th>
                        <th style={{ textAlign: 'left', padding: '16px 24px' }}>ACTIVIDAD</th>
                        <th style={{ textAlign: 'left', padding: '16px 24px' }}>IP</th>
                      </tr>
                    </thead>
                    <tbody>
                      {auditoriaActividades.length > 0 ? (
                        auditoriaActividades.map((actividad, idx) => {
                          ...
                          return (
                            <tr key={idx}>
                              <td style={{ padding: '16px 24px', color: '#6b7280', fontSize: '14px' }}>{fechaStr}</td>
                              <td style={{ padding: '16px 24px', fontWeight: '600', color: '#111827' }}>{actividad.usuario}</td>
                              <td style={{ padding: '16px 24px' }}>{actividad.actividad}</td>
                              <td style={{ padding: '16px 24px', fontFamily: 'monospace', color: '#4b5563' }}>{actividad.ip}</td>
                            </tr>
                          )
                        })
                      ) : (
                        <tr>
                          <td colSpan="4" style={{ textAlign: 'center', padding: '32px', color: '#6b7280' }}>
                            {auditoriaLoading ? 'Cargando registros...' : 'No hay registros de auditoría recientes.'}
                          </td>
                        </tr>
                      )}
                    </tbody>
            ...
            <div className="panel-card" style={{ background: 'transparent', boxShadow: 'none', padding: 0 }}>
              <div className="panel-card-header" style={{ background: '#fff', borderRadius: '12px', padding: '24px', marginBottom: '24px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', flexWrap: 'wrap', gap: '20px' }}>
                  ...
                  <form onSubmit={handleFiltrarVentas} style={{ display: 'flex', gap: '15px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                    <div className="form-group" style={{ marginBottom: 0 }}>
                      <label>Fecha Inicio</label>
                      <input 
                        type="date" 
                        value={filtroVentas.fechaInicio}
                        onChange={(e) => setFiltroVentas({...filtroVentas, fechaInicio: e.target.value})}
                      />
                    </div>
                    <div className="form-group" style={{ marginBottom: 0 }}>
              ...
              {reporteVentasError && <div className="error-message">{reporteVentasError}</div>}
              {reporteVentasLoading ? (
                ...
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', marginBottom: '24px' }}>
                    <div style={{ background: '#fff', padding: '24px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', border: '1px solid #f3f4f6' }}>
                      <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Total Ventas</h3>
                    ...
                    <div style={{ background: '#fff', padding: '24px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', border: '1px solid #f3f4f6' }}>
                      <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Promedio Venta Diaria</h3>
                      <p style={{ fontSize: '32px', fontWeight: '700', color: '#8b5cf6' }}>
                        ${(reporteVentas.promedioVentaDiaria || 0).toLocaleString()}
                      </p>
                    </div>
                  </div>
                  ...
                  {reporteVentas.ventasPorDia && reporteVentas.ventasPorDia.length > 0 && (
                    <div className="panel-card" style={{ padding: '0', overflow: 'hidden' }}>
                      <div className="panel-card-header" style={{ padding: '20px 24px', borderBottom: '1px solid #e5e7eb' }}>
                        <h3 style={{ fontSize: '16px', fontWeight: '600', margin: 0 }}>Ventas Detalladas por Día</h3>
                      </div>
                      <div className="table-container">
                        <table className="data-table">
                          <thead>
                            <tr>
                              <th style={{ textAlign: 'left', padding: '16px 24px' }}>FECHA</th>
                              <th style={{ textAlign: 'right', padding: '16px 24px' }}>CANTIDAD RESERVAS</th>
                              <th style={{ textAlign: 'right', padding: '16px 24px' }}>MONTO RECAUDADO</th>
                            </tr>
                          </thead>
                          <tbody>
                            {reporteVentas.ventasPorDia.map((venta, idx) => {
                              ...
                              return (
                                <tr key={idx}>
                                  <td style={{ padding: '16px 24px', fontWeight: '500' }}>{fechaFormateada}</td>
                                  <td style={{ textAlign: 'right', padding: '16px 24px' }}>{venta.cantidadReservas || 0}</td>
                                  <td style={{ textAlign: 'right', padding: '16px 24px', fontWeight: '600', color: '#10b981' }}>
                                    ${(venta.montoRecaudado || 0).toLocaleString()}
                                  </td>
                                </tr>
                              )
                            })}
          ...
          {activeSection === 'prestadores' && (
            <div className="panel-card" style={{ background: 'transparent', boxShadow: 'none', padding: 0 }}>
              ...
              {prestadoresError && <div className="error-message">{prestadoresError}</div>}
              {prestadoresLoading ? (
                ...
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', marginBottom: '24px' }}>
                    <div style={{ background: '#fff', padding: '24px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', border: '1px solid #f3f4f6' }}>
                      <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Empleados Presentes Hoy</h3>
                      <p style={{ fontSize: '36px', fontWeight: '700', color: '#0ea5e9' }}>{prestadores.empleadosPresentes}</p>
                    </div>
                  </div>

                  {prestadores.turnosPorDepartamento && prestadores.turnosPorDepartamento.length > 0 && (
                    <div className="panel-card" style={{ padding: '0', overflow: 'hidden' }}>
                      <div className="panel-card-header" style={{ padding: '20px 24px', borderBottom: '1px solid #e5e7eb' }}>
                        <h3 style={{ fontSize: '16px', fontWeight: '600', margin: 0 }}>Turnos por Departamento</h3>
                      </div>
                      <div className="table-container">
                        <table className="data-table">
                          <thead>
                            <tr>
                              <th style={{ textAlign: 'left', padding: '16px 24px' }}>DEPARTAMENTO</th>
                              <th style={{ textAlign: 'right', padding: '16px 24px' }}>EMPLEADOS ASIGNADOS</th>
                            </tr>
                          </thead>
                          <tbody>
                            {prestadores.turnosPorDepartamento.map((turno, idx) => (
                              <tr key={idx}>
                                <td style={{ padding: '16px 24px', fontWeight: '500' }}>{turno.departamento}</td>
                                <td style={{ textAlign: 'right', padding: '16px 24px' }}>
                                  <span className="badge badge-gray" style={{ fontSize: '14px' }}>{turno.cantidad}</span>
                                </td>
                              </tr>
                            ))}
                          </tbody>
            ...
            <>
              ...
              <div className="panel-card">
                <div className="panel-card-header">
                  ...
                  <button className="add-button" onClick={() => setShowConfigForm(p => !p)}>
                ...
                {configLoading && !config.capacidadMaxima ? (
                  ...
                  <div className="table-container">
                    <table className="data-table">
                      ...
                      <tbody>
                        <tr>
                          <td><strong>{config.capacidadMaxima}</strong> personas</td>
              ...
              {/* ── MODAL de edición ── */}
              {showConfigForm && (
                <div className="modal-backdrop" onClick={() => setShowConfigForm(false)}>
                  <div className="modal-box" onClick={e => e.stopPropagation()}>
                    <div className="modal-header">
                      ...
                      <button className="modal-close" onClick={() => setShowConfigForm(false)}>✕</button>
                    </div>
                    ...
                    <form onSubmit={handleConfigSubmit} className="config-form">
                      <div className="form-row-3">
                        ...
                        <div className="form-group">
                          ...
                          <input type="number" id="horaApertura" name="horaApertura"
                            value={config.horaApertura} onChange={handleConfigChange} min="0" max="23" required />
                          <span className="input-hint">Formato 24h (0–23)</span>
                        </div>
                        <div className="form-group">
                          <label htmlFor="horaCierre">Hora de cierre</label>
                          <input type="number" id="horaCierre" name="horaCierre"
                            value={config.horaCierre} onChange={handleConfigChange} min="0" max="23" required />
                          <span className="input-hint">Formato 24h (0–23)</span>
                        </div>
                      </div>
                      <div className="form-row">
                        ...
                        <div className="form-group">
                          ...
                          <input type="number" id="margenGanancia" name="margenGanancia"
                            value={config.margenGanancia} onChange={handleConfigChange} min="0" step="0.5" required />
                        </div>
                      </div>
                      <div className="modal-footer">
                        <button type="button" className="btn-cancel" onClick={() => setShowConfigForm(false)}>
                          Cancelar
                        </button>
                        <button type="submit" className="submit-button" disabled={configLoading}>
                    ...
                    <div className="form-row">
                      <div className="form-group">
                        ...
                        <input type="date" id="periodoInicio" name="periodoInicio"
                          value={formData.periodoInicio} onChange={handleFormChange}
                          min={new Date().toISOString().split('T')[0]} required />
                      </div>
                      <div className="form-group">
                        <label htmlFor="periodoFin">Fecha fin</label>
                        <input type="date" id="periodoFin" name="periodoFin"
                          value={formData.periodoFin} onChange={handleFormChange}
                          min={formData.periodoInicio || new Date().toISOString().split('T')[0]} required />
                      </div>
                    </div>
                    <div className="checkbox-row">
                      <label className="checkbox-label">
                        <input type="checkbox" name="consideraFestivos" checked={formData.consideraFestivos} onChange={handleFormChange} />
                        <span>Considerar festivos</span>
                      </label>
                      <label className="checkbox-label">
                        <input type="checkbox" name="consideraTendencias" checked={formData.consideraTendencias} onChange={handleFormChange} />
              ...
              {predLoading ? (
                ...
                <div className="table-container">
                  <table className="data-table">
                    <thead>
                      <tr>
                        <th>#</th><th>PERÍODO INICIO</th><th>PERÍODO FIN</th>
                        <th>FESTIVOS</th><th>TENDENCIAS</th><th>ESTADO</th>
                      </tr>
                    </thead>
                    <tbody>
                      {predicciones.map((pred, i) => (
                        <tr key={pred.id || i}>
                          <td>{i + 1}</td>
                          <td>{new Date(pred.periodoInicio).toLocaleDateString('es-ES')}</td>
                          <td>{new Date(pred.periodoFin).toLocaleDateString('es-ES')}</td>
                          <td><span className={`badge ${pred.consideraFestivos ? 'badge-green' : 'badge-gray'}`}>{pred.consideraFestivos ? 'Sí' : 'No'}</span></td>
                          <td><span className={`badge ${pred.consideraTendencias ? 'badge-green' : 'badge-gray'}`}>{pred.consideraTendencias ? 'Sí' : 'No'}</span></td>
                          <td><span className="availability-badge available">Activo</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {activeSection === 'administradores' && (
            <div className="panel-card">
              ...
              {adminsError && <div className="error-message">{adminsError}</div>}
                ...
                <div className="table-container">
                  <table className="data-table">
                    ...
                    <tbody>
                      {admins.map((admin) => (
                        <tr key={admin.id}>
                          <td><strong>{admin.usuarioNombre}</strong></td>
                          <td>{admin.usuarioEmail}</td>
                          <td><span className="badge badge-gray">{admin.nivelAcceso}</span></td>
                          <td>{admin.areaResponsabilidad || '-'}</td>
                          <td>
                            <span className={`availability-badge ${admin.activo ? 'available' : 'full'}`}>
                          ...
                          <td>
                            <button 
                              ...
                              style={{ width: '32px', height: '32px', borderRadius: '6px', color: '#dc2626', borderColor: '#fca5a5', background: '#fee2e2' }}
              ...
              {showAdminForm && (
                <div className="modal-backdrop" onClick={() => setShowAdminForm(false)}>
                  <div className="modal-box" onClick={e => e.stopPropagation()}>
                    <div className="modal-header">
                      <div>
                      ...
                      <button className="modal-close" onClick={() => setShowAdminForm(false)}>✕</button>
                    </div>
                    
                    {adminFormError && <div className="error-message" style={{ margin: '16px 28px 0' }}>{adminFormError}</div>}
                      ...
                      <div className="form-row">
                        <div className="form-group">
                          ...
                          <input type="text" id="nivelAcceso" name="nivelAcceso"
                            value={adminFormData.nivelAcceso} onChange={handleAdminFormChange} disabled />
                          <span className="input-hint">Asignado automáticamente</span>
                        </div>
                        <div className="form-group">
                          <label htmlFor="areaResponsabilidad">Área Responsabilidad</label>
                          <input type="text" id="areaResponsabilidad" name="areaResponsabilidad"
                            value={adminFormData.areaResponsabilidad} onChange={handleAdminFormChange} disabled />
                          <span className="input-hint">Asignado automáticamente</span>
                        </div>
                      </div>

                      <div className="checkbox-row" style={{ padding: '4px 0 10px' }}>
                        <label className="checkbox-label" style={{ opacity: 0.7, cursor: 'not-allowed' }}>
                          <input type="checkbox" name="activo" checked={adminFormData.activo} disabled />
                          <span>Usuario Activo (por defecto)</span>
                        </label>
                      </div>

                      <div className="modal-footer">
                        <button type="button" className="btn-cancel" onClick={() => setShowAdminForm(false)}>Cancelar</button>
                        <button type="submit" className="submit-button" disabled={adminFormLoading}>
            ...
            <div className="panel-card">
              <div className="panel-card-header">
                ...
                <form onSubmit={handleSearchAdmins} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  ...
                  />
                  <button type="submit" className="submit-button" style={{ padding: '8px 16px', borderRadius: '8px' }}>
                    Buscar
                  </button>
                </form>
              </div>

              {activeAdminsError && <div className="error-message">{activeAdminsError}</div>}
              
              {activeAdminsLoading ? (
                <div className="loading-state">Cargando administradores activos...</div>
        ...
        <footer className="adm-footer">
          © 2026 RestaurantApp · Panel de Administración · Todos los derechos reservados.
        </footer>
      </div>
    </div>
  )
